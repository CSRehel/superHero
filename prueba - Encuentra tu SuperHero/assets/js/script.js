$(document).ready(function(){
    $('form').submit(function(event){
        ...
        //se valida que el dato ingresado sea numerico, mayor a cero y menor al ultimo dato de la lista de superheros
        if($.isNumeric(heroInput) && (heroInput > 0 && heroInput <= 731)){
            $.ajax({
                ...
                //se crea un arreglo vacio para las estadísticas
                let estadisticas = []
                let acc = 0
                ...
                for (const d in data.powerstats) {
                    ...
                    // se llena el arreglo estadisticas con los parametros que requiere el gráfico
                    estadisticas.push({
                        label: d,
                        y: valores[acc]
                    })
